...
def main() -> None:
    ...
    skipped_non_active = len(users) - len(active_users)
    print(f"Active users to audit: {len(active_users)} (skipped non-active: {skipped_non_active})")
    ...
    for user in active_users:
        try:
            workspace_email_candidates = generate_workspace_primary_email_candidates(user, cfg.google_workspace_domain)
        except ValueError:
            results.append(
                {
                    "kos_user_id": user.get("id"),
                    ...
                    "missing_groups": cfg.google_workspace_groups,
                    "in_all_groups": False,
        ...
        expected_workspace_email = workspace_email_candidates[0]
        ...
        if not exists:
            ...
            for candidate in workspace_email_candidates:
                normalized_candidate = _normalize_email_for_match(candidate, cfg.google_workspace_domain)
        ...
        result["expected_workspace_email"] = expected_workspace_email
        result["email_match_source"] = email_match_source
        ...
        json.dumps(
            {
                ...
                "total_non_active_skipped": skipped_non_active,
